...
export class EntityRepository {
    ...
    // 加载所有实体 (游戏启动时)
    static async loadAll(): Promise<IEntity[]> {
        const db = await getDatabase();
        const docs = await db.entities.find().exec();
        return docs.map(doc => doc.toJSON() as IEntity);
    }

    // 根据 ID 加载
    static async loadById(id: EntityID): Promise<IEntity | null> {
        const db = await getDatabase();
        const doc = await db.entities.findOne(id).exec();
        return doc ? (doc.toJSON() as IEntity) : null;
    }
